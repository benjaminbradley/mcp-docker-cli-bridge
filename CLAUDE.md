...
**MCP Docker CLI Bridge** — a single-file MCP server (`server.py`) that runs inside a Docker container during development and exposes allowlisted CLI commands (pytest, ruff, mypy, etc.) as MCP tools over Streamable HTTP. Claude Code connects to it over a Docker bridge network and calls tools autonomously.
...
- **Named recipe allowlist** in `commands.json` (mounted read-only). Operator defines commands; caller picks a name.
...
2. **MCP Tool Provider** — dynamically registers one MCP tool per allowlist entry; tools with `allow_extra_args: true` expose an `args: string[]` schema parameter, others get empty schema
...
5. **Executor** — allowlist lookup → build argv → `subprocess.run(shell=False, capture_output=True, text=True, timeout=effective_timeout, cwd=entry.cwd)` → `CommandResult`
...
│   ├── commands.json         # allowlist (mounted :ro)
...
| `BRIDGE_COMMANDS_FILE` | `/bridge/commands.json` | allowlist path |
